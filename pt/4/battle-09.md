...
1. Adicione uma declaração `else`. Se nosso zumbi perdeu:

  a. Incremente `myZombie` `lossCount`.

  b. Incremente `enemyZombie` `winCount`.

2. Fora da declaração else, execute o código da função `_triggerCooldown` em `myZombie`. Desta maneira o zumbi só poderá atacar uma vez por dia.
