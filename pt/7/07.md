---
title: Aplicação de cartões proibidos
...
Agora que definimos uma lista das cartas que estão proibidas em nosso modo de jogo, temos que escrever a lógica para verificar as cartas do baralho de cada jogador e remover as que não são permitidas.

Uma vez que não é simples excluir um elemento em um array no Solidity, é mais fácil simplesmente criar um novo array na memória e copiar todos os cartões "permitidos".

Vamos examinar as peças de que precisamos para fazer isso.

## Iterando através de uma lista de jogadores

Lembre-se de que nossa estrutura `gameState` agora contém todos os detalhes do estado atual do jogo, incluindo jogadores e suas cartas.

Os detalhes de ambos os jogadores são armazenados em `gameState.playerStates`.

Podemos usar o seguinte loop `for` para iterar em cada jogador:

```
for (uint i = 0; i < gameState.playerStates.length; i++) {
    // Faça alguma ação para cada jogador, gameState.playerStates[i]
}
```

## Declaring a new array of cards

`CardInstance` é a estrutura de ZBGameMode que armazena os dados dos cartões:
...
Para fazer uma nova matriz de cartas para cada jogador, para que possamos copiar as cartas permitidas uma a uma, podemos fazer o seguinte:

```
// Crie uma matriz de CardInstances com um comprimento de 30 (um baralho de 30 cartas):
CardInstance[] memory newCards = new CardInstance[](30);
```

## Coloque isto à prova

1. Crie um loop `for` que itera através de cada jogador no` gameState`

2. Para cada jogador, crie um array de `CardInstance`s chamado` newCards`. O comprimento desta matriz deve ser `gameState.playerStates [i] .cardsInDeck.length`

3. Crie um novo `uint` chamado` cardCount` e defina-o igual a `0`.
