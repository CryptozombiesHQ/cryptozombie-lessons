---
title: "Etapa final: emissão de alterações"
...
Depois que nosso loop `for` aninhado terminar de funcionar, teremos trocado os cartões de ambos os jogadores por um novo conjunto que não inclui nenhum cartão banido. (Se o jogador não tiver nenhum item em seu deck desde o início, nenhuma alteração terá sido feita).

Também fizemos algumas alterações nos frascos de defesa e goo iniciais de cada jogador.

Mas lembre-se de que `changes` é simplesmente um tipo de dado local armazenado na memória. Para completar nosso modo de jogo, precisaremos passar esses dados de volta para a lógica principal do jogo.

Isso pode ser feito simplesmente com `changes.emit ()`:

```
changes.emit();
```

Como o nome descreve, ele emitirá um `evento`, que a lógica central do jogo está escutando.

Este evento contém os dados de todas as alterações que queremos fazer, em um formato semelhante a `serializedGameState`.
## Coloque isto à prova

1. Termine nossa função com`changes.emit()`.
