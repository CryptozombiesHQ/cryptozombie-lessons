---
title: Forçando Cartões Banidos (Cont.)
    ...
    startingCode:
      "examplegame.sol": |
        ...
        contract ExampleGame is ZBGameMode  {
            ...
            constructor() public {
                // Definir regras para quais cartas são banidas do jogo
            ...
            function beforeMatchStart(bytes serializedGameState) external {
                ...
                // Percorra o deck de cada jogador e modifique-o para remover as cartas proibidas
...
Continuando com o capítulo anterior, agora queremos iterar cada carta do baralho do jogador e verificar se elas foram banidas.

## Looping pelas cartas no deck de um jogador

Aqui está um exemplo de como você iteraria cada carta nos baralhos de ambos os jogadores e mudaria todas para "Slab" - uma carta favorita dos fãs 😉
...
```
Como você pode ver, usamos um loop `for` aninhado para primeiro iterar pela lista de cada jogador e, em seguida, para cada jogador, iterar por uma lista de cada uma de suas cartas.

> Nota: Mudar um `mouldName` de um card no deck de um jogador irá, na verdade, trocar aquele card inteiramente pelo card denominado Slab. Não é apenas um nome de exibição.

## Coloque isto à prova

1. Crie um segundo loop `for` que itera através de todos os` cardsInDeck` para cada jogador, como acima

2. Crie uma instrução `if` que chama uma função` isLegalCard` e passa `gameState.playerStates [i] .cardsInDeck [j]`. Definiremos essa função no próximo capítulo.

3. Dentro da instrução `if`, copie o cartão para nosso novo array definindo` newCards [cardCount] `igual a` gameState.playerStates [i] .cardsInDeck [j] `.

4. Use `cardCount ++` para incrementar `cardCount`.