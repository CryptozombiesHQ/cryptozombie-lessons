---
title: Lendo dados do GameState
...
---
O `GameState` que inicializamos no Capítulo 3 agora pode ser usado para ler dados do estado atual do jogo.

No final de `beforeMatchStart ()`, precisaremos passar os dados de volta para o servidor do jogo como um `SerializedGameState`.

Para fazer isso, precisamos declarar um tipo de dado customizado chamado `ZBSerializer.SerializedGameStateChanges`, que rastreará todas as mudanças que fizermos.

```
ZBSerializer.SerializedGameStateChanges memory changes;
```

`changes` é, como o nome indica, onde faremos todas as mudanças no estado do jogo que retornaremos no final desta função.

Depois de declarar isso, também precisaremos inicializá-lo:

```
changes.init();
```
Isso o deixa pronto para começar a armazenar dados. Ao contrário de `gameState`,` changes.init () `não requer nenhum argumento.

## Coloque isto à prova

1. Declare um `ZBSerializer.SerializedGameStateChanges` em `memory` chamado `changes`.
