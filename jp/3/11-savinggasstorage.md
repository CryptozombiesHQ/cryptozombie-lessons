...
他のほとんどのプログラム言語では、データセットを繰り返すのは費用が高くなる。しかしSolidityでは`view`関数にガスのコストがかからないから、`external view`関数を使う方が、`storage`を使うよりも安くなるのだ。（ガスコストはユーザーが実際にお金を出して買うものだということをわすれるなよ！）。
