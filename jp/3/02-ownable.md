...
ではどうすれば良いかというと、一般的には`Ownable`（所有権）というコントラクトを作成するのだ。これはオーナー（所有者）が特別な権限を持つことを意味するものだ。
...
- コンストラクタ： `function Ownable()`は**_コントラクタ_**だ。これは特別な関数で、コントラクトと同じ名前だ。コントラクトが最初に作成された時に、1度だけ実行されるぞ。
...
1. `ownable.sol`の内容を我々のコードを`import` するように変更せよ。なに、やり方を忘れてしまっただと？その場合は`zombiefeeding.sol`を見て思い出すのだ。

2. `ZombieFactory`コントラクトを編集して `Ownable`から継承させよ。やり方を忘れていたら`zombiefeeding.sol` を見て思い出すように。
