...
それ以外にも、同じデータ型を一つにクラスタして（つまり、structの中で一つにまとめて）、Solidityのstorageスペースを最小限に抑えることも可能だ。例えば、`uint c; uint32 a; uint32 b;`フィールドは、`uint32 a; uint c; uint32 b;`よりもコストが低くなる。なぜなら`uint32`フィールドでクラスタにまとめることできるからだ。
