...
良し！君は*Munchkin*のスマートコントラクトを終わらせたわけだ！

カスタムゲームモードを作るのはとてもシンプルだろ？

じゃあ次のゲームに移るぞ！

_**Singleton:** 30枚のデッキを作る際に同じカードがデッキ内にあってはいけない._

先ず始める為に、我々が'munchkin.sol'をフォーク（コピーを作って）して'singleton.sol'と名前を付けておいた。

繰り返しになるが、これが1番簡単にカスタムゲームモードを作り始める方法なんだ。1から作り始めるのではなく、既にあるコントラクトをコピーして編集するのさ。

> Note: もし君が前のゲームに戻ってコードを参照したいのであれば、'munchkin.sol'はタブに残しておいた。

*Singleton* は*Munchkin*と比べると少しトリッキーなゲームだ。*Munchkin*ではとてもシンプルなルールで、'isLegalCard()'を使うだけで禁止かどうかを試せた。

*Singleton*ではカードが使えるかを確認する為に既にデッキの中にある他のカード達と比べなければならない。この為にはもう一つ新たな'for'ループが必要になる。

ロジックは以下のようになる。各カードにつき...

1. 'cardAlreadyInDeck'という名前の'bool'をfalseと定義する。

2. このカードがデッキの中にあるかどうかを確認する為に、既に'newCards'に追加されたカード達をループしていく。もしあるようだったら'cardAlreadyInDeck'を'true'に変更する。

3. ループの終わりに'cardAlreadyInDeck'がまだ'false'かどうかを確認する。もしそうであればこのカードを'newCards'に追加する。

これらをこのチャプターと次のチャプターで書いていくぞ！

## さあテストだ！

先ず始める前に、コントラクトの名前を変更しよう。その後上に書いてあるロジックを書いていくぞ！

1. コントラクトの名前を`Munchkin` から `Singleton`に変えてくれ！

2. 'cardAlreadyInDeck'という名前の'bool'を定義して、'false'に設定してくれ。

3. 上のステップ2にある新しい'for' loopは次のチャプターで書いていく。今はステップ3を先に書いて行こう：

既にある'if'文のコンディションを'cardAlreadyInDeck'が'false'かを確認するようにしてくれ。これは`!cardAlreadyInDeck`で行う事が出来る。

4. 最後に、使わなくなった'isLegalCard()'を削除してくれ！
