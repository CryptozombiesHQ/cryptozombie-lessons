      ...
      "munchkin.sol": |
        ...
        contract Munchkin is ZBGameMode  {
            ...
            function beforeMatchStart(bytes serializedGameState) external {
                ...
                // 1. nested for　ループをここにコピー＆ペースとしてくれ！

                changes.emit();

            }

            // 2. ここに isLegalCard() をDefineしてくれ
...
---
コントラクトのシェルが出来たな！今度は各プレイヤーのカードをルールに沿って選別していくロジックを書いて行こう。

'Munchkin'のルールによれば、我々はコストが2以上のゾンビカードを弾かなければならない。

先ず最初に各プレイヤーが持っているカードを確認していく。前のレッスンでこの 'for'ループを使ったのを思い出してくれ：
...
このロジックはカスタムゲームモードで各プレイヤーのデッキがカスタムルールに沿っているかを確認する時によく使う事になる。君もカスタムゲームを作る時にこれをコピー＆ペースとする事になるだろう。

## さあテストだ！

1. カードの選別をする為に、上のnested 'for' loopをコードにコピー＆ペースとしてくれ。

2. 'isLegalCard'という名前のfunctionを作ってくれ。このfunctionは'bool'をreturnし、'card'という名前の'CardInstance'パラメーターを1つ持っている。functionを'internal'と'view'に設定してくれ。

functionのbodyはそのまま空欄にしておいてくれ。
