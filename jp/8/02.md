      ...
      "munchkin.sol": |
        ...
        contract Munchkin is ZBGameMode  {
            ...
            function beforeMatchStart(bytes serializedGameState) external {
                ...
                // 1. ネスト化された`for`ループをここにコピー＆ペーストするのだ

                changes.emit();

            }

            // 2. isLegalCard()をここに定義するのだ
...
コントラクトのシェルはできているから、今度はルールセットに基づいてカードをフィルタリングするカスタムロジックを書いていこう。

`Munchkin`のルールに従うと、Gooコストが２より多いカードを除外するんだったな。

最初のステップとして、各プレイヤーのカードリストを反復しよう。前のレッスンで次の`for`ループをやったことを思い出したな？:
...
これは各プレイヤーのデッキがカスタムゲームルールに適合しているかを検証するための、共通のロジック構造みたいなものとなる。お主のカスタムゲームモードにいくらでもコピー＆ペーストして使っていいからな。

## さあテストだ

1. 上のネスト化された`for`ループをコントラクト内にコピー&ペーストし、ルールに適合するカードのフィルタリング機能を追加できるようにせよ。

2. `isLegalCard`という名の関数を定義せよ。この関数は`card`という名の１つの引数`CardInstance`を受け取り、`bool`を返却する。関数のプロパティは`internal`及び`view`とすること。
