      ...
      "zombiepile.sol": |
        ...
        contract Singleton is ZBGameMode  {
            ...
            function beforeMatchStart(bytes serializedGameState) external {
                ...
                // 3. プレイヤー１とプレイヤー２のカードのarrayとcardCountを定義出来るよう、
                //2でカットしたものをここにペースとして変えてくれ。
...
おめでとう！*Singleton*のロジックはこれで終わりだ。二つ目のゲームも無事に実装出来たな。

次は三つ目のゲームモード、 *Zombie Pile*を実装するぞ。また繰り返しになるが、前のコントラクトからフォークして名前を'zombiepile.sol'に変更しておいた。前のコントラクトを参照したければ'singleton.sol'はタブにあるぞ。

ゲームモードの概要のおさらいはこんな感じだ：

> **Zombie Pile**: ゲームの始めに各プレイヤーのカードを纏めてシャッフルし、ランダムに配り直す。

我々がこれを実装する方法は実際にカードを纏めてシャッフルして配り直すのとは少し違う。

その代わりに我々は各プレイヤーのカードをループして行く（前に実装したのと同じ様に）。各カード選択すると同時に、コインをフリップする（0か1の乱数を出す)。もし乱数が0ならプレイヤー１のデッキにそのカードは行き、1ならプレイヤー２のデッキにいく事になる。

この方法の方が簡単な上に、最終的な結果は同じになる。

前の実装の仕方とは少し違う為、'for' ループの*前に*'newCards'のインスタンスがプレイヤー１とプレイヤー２のどちらにも必要になる。こうする事によってループの中でどちらのデッキにもカードを追加出来る。

## さあテストだ！

1. 先ずはコントラクトの名前を`ZombiePile`に変えてくれ

2. `CardInstance[] memory newCards newCards`と`uint cardCount` をループの前に移動させなければならない. この二行をカットしてくれ。

3. 'for' ループの前にこれらをペーストしてくれ。今度はこれらを書き換えてデッキを二つ作らなければならない。

  a. `newCards`と呼ぶ代わりに, `player1Cards`と`player2Cards`という名前のarrayを作ってくれ。

  b. `player1Cards` の長さは`gameState.playerStates[0].cardsInDeck.length`, で`player2Cards`の長さは`gameState.playerStates[1].cardsInDeck.length`でなければならない。

  > Note: 各プレイヤーは自分がゲームに持ち込んだカードと同じ枚数を受け取る事になる為、正確にはこれは全く同じシャッフルでは無い。これは一人のプレイヤーがカードを1枚しか持ち込まずに一人が15、もう一人が16枚のデッキにならない様にしている。
4. `player1Cards`と`player2Cards`を定義した後は, 2つの `uint`s: `player1CardCount`と`player2CardCount`が必要だ。どちらも0にセットしてくれ(これらは'cardCount'のインスタンスを置き換える).
