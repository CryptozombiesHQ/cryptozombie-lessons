...
material:
  editor:
    ...
    startingCode:
      ...
      "ZBGameMode.sol": |
        // このファイルいは君が参考にするためだけにあり、コンパイルするのに必要な
...
このレッスンでは少し駆け足で**3つのカスタムゲームモード**を見て行こう！

いくつかのコード例を見て行く事で君がカスタムデッキルールを作る事に慣れて行ければ最高だ！

私達が作っていく3つのゲームとその要約が以下になる。_このゲームを考えてくれた我々のコミュニティーメンバー、Brook Boyd (aka MidnightLightning)に感謝の言葉を送る。_

1. **Munchkin**: 30枚のデッキを作る際にコストが2以下のゾンビしか選べなくなる。

2. **Singleton**: 30枚のデッキを作る際に*全てのカードが違う名前でなければならない。* (例： 最大1枚しか各カードをデッキに入れられない)。

3. **Zombie Pile**: ゲームが始まる際に**二人のプレイヤーのデッキを纏めてシャッフルし**、ランダムに各プレイヤーに配られる。

先ずは最初のゲームモード、**Munchkin**を作って行こう！

## さあ、スタートだ！

前のレッスンで習った大事な部分だけが埋められているほとんど空欄のコントラクトから始めてみよう。

カスタムゲームを作る時大体の場合は既存のContractをフォークかコピーして編集する所から始める。これは君が将来カスタムゲームを作る時にテンプレートとして使える最低限のコードだ。このレッスンが終わったら3つの内どのゲームのコードでも君の作るゲームモードのテンプレとして使ってくれ！

先ずは私達の作る新しいゲームに名前を付けなければならない。次のチャプターからゲームモードのロジックを書いていくぞ。

## さあテストだ！

1. コントラクトの名前を'ExampleGame'から'Munchkin'に変えてみてくれ！
