...
このレッスンでは、**３つの様々なカスタムゲームモード** の実装方法を素早く見ていこう。

次々にいくつかの例を見ていくことで、カスタムデッキルールのゲームモードを作るプロセスがかなりやり易いものになるはずだ。

ここに３つのモードを実装していく。簡単な概要も記しておいたぞ。 _このゲームモードのアイディアを提供してくれたコミュニティメンバー・Brooks Boyd (aka MidnightLightning)に感謝します。_

1. **Munchkin**: 30枚のカードでデッキを組む場合に、*Gooコスト２もしくはそれ未満のカード* のみを選ぶことができる。

2. **Singleton**: 30枚のカードでデッキを組む場合に、*デッキ内で１つのカード名を複数カードが共有することはできない* (つまりデッキ内で各カードを１枚ずつしか持つことができない).

3. **Zombie Pile**: バトルのスタート時に、*両プレイヤーのカードが合わせてシャッフルされ* 、両者は自分と相手のカードをミックスしたデッキをランダムに受け取る。

では最初のモード・ **Munchkin** の実装からレッスンを始めていくぞ。

## スタートしよう

前のレッスンではコアロジックを学んだな、これが含まれている(ほぼ)空っぽのコントラクトシェルから始めていこう。

通常新たなゲームモードを作ろうとする場合は、既存のコントラクトをフォーク/コピーしてそれを変更したいと思うだろう。もし今後スクラッチから作りたい場合は、この最小限のコントラクトをテンプレートとして使ってみよう。このレッスンを修了したら、この３つのゲームモードをお主のゲームモードのテンプレートとして使えるようになるのだ。

まずは我らがコントラクトに新たな名前をつけて、次のチャプターでゲームモードのカスタムロジックに取り掛かろう。
