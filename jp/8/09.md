...
'for'ループの最後終わりでは、カードは`player1Cards`か'player2Cards`のどちらかに追加されている事になる。

これらの変更を実際にクライアントに教える為に、最後のステップで我々は`changes.changePlayerCardsInDeck`を各プレイヤーの為にコールしなければならない。

## さあテストだ！

1. `changes.changePlayerCardsInDeck(Player(i), newCards, cardCount);`の行をカット(またはコピーして削除)してくれ。

2. 'for'ループの**外側に**ペースとしてくれ。このループは**どちらの**プレイヤーのデッキも完成させてから終わらなければならない。

  我々はこのコードを各プレイヤーのために二回行わなければならない。

  a. 最初は`Player(i)`の代わりに`Player.Player1`を最初のパラメーターとしなければならない。次に二つ目と三つ目のパラメーターは'newCards'と'cardCount'の代わりに'player1Cards`と`player1CardCount`になる。

  b. これをコピー・ペースとする。ただし今回はプレイヤー２のパラメーターを入れる。
