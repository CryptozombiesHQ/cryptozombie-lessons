...
---
*Munchkin*を完成させる最後のステップは'isLegalCard()'の中身を書く事だ！

とても簡単だろ？

*Munchkin*のルールを思い出してくれ。我々は**こそとが2以上のカードを禁止したい**。

一番効率的な方法を見つける為に、先ずは下の'CardInstance'を見てくれ。
...
見てわかる通り、'gooCost'のプロパティがある。つまりカードが禁止されているかを確かめる為には、シンプルにこの'card.gooCost'が2以下かどうかを確認すれば良いんだ。

## さあテストだ！

'isLegalCard()'は'bool'をリターンする為、このfunctionは'return'を含んだ一文だけで書く事が出来る。

このfunctionは'card.gooCost'が2以下なら'true'を、それ以外なら'false'をリターンしなければならない。

1. functionを1行で埋めてみてくれ！
