...
*Munchkin* の最後のステップは、`isLegalCard()`の関数定義を埋めることだ。そしたらこのゲームモードの実装は完成するぞ！

かなり簡単だろ？

 *Munchkin* のルールは、 **Gooコストが２より多いカードを禁止** するものだったということを思い出すのだ。

`CardInstance`をチェックして、これらのカードをフィルタリングするベストな方法を考えよう:
...
見ての通り、`gooCost` というプロパティがある。 *Munchkin* においてカードがルールに適しているかをテストするには、`card.gooCost`が２以下であるかチェックするだけで良いのだ。

## さあテストだ

`isLegalCard()`は`bool`を返すから、この関数定義は`return`ステートメントのコード一行で埋めることができる。

この関数は`card.gooCost`が２以下である場合は`true`、そうでなければfalseを返却しなくてはならない。

1. `return`ステートメントで関数定義を埋めよ。
