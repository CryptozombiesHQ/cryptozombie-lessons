...
良し！2つの空っぽのデッキが各プレイヤー用に出来たな。

次は各プレイヤーのデッキのカードを一枚一枚見ていき、コインをフリップする様に乱数を出し、これらを使ってどっちのプレイヤーが各カードを手に入れるかを決めていく。

Solidityでコインをフリップする方法は以下の通りだ：

```
uint nonce = 0;
for (uint i = 0; i < 10; i++) {
    // `% 2` は2で割って余りを取るという意味。これによって'rand'は
    // `0`か`1`になる
    ...
    // 毎回別のrandが出る様にnonceを変化させなければならない
    // そうでなければ毎回同じrandが出てしまう
    nonce++;
}

```

もし君が乱数についておさらいが必要なら、CryptoZombieの<a href="https://cryptozombies.io/en/lesson/4/chapter/4" target=_blank>Lesson 4 Chapter 4</a> を確認してくれ。

> ノート・おさらい: この様な乱数の出し方はギャンブルゲームなどでは使われてはいけない。ブロックの作成者が乱数の値によってはトランザクションをブロードキャストせずに、自分にとって都合のいい数字が出るまで乱数を出し続けられるからだ。

## さあテストだ！

1. 先ずは'for' ループの中のロジックを全部消してしまおう。

2. 最初のステップは'rand'という名前の'0'か'1'の'uint'を作り出す所からだ。ただ今回は毎回どちらかのプレイヤーカウントが変わる事が分かっているので、'nonce'の代わりに`player1CardCount + player2CardCount` を使ってみよう。

3. 乱数が出来た所でカードをどちらかのプレイヤーのデッキに追加していこう。

  a. 'rand'が'0'かを確認する'if/else'文を作ってくれ。
  
  b. もし'true'なのであれば、カードをプレイヤー１のデッキに`player1Cards[player1CardCount]`を`gameState.playerStates[i].cardsInDeck[j]`にセットして追加してくれ。

  c. `player1CardCount`を`player1CardCount++`を使って大きくしてくれ。

  d. `else`の中でこれの反対をプレイヤー２の為に書いてくれ。**Player 2**のデッキにカードを追加し`player2CardCount`を大きくする。
