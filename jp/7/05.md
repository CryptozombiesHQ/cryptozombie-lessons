...
`changes`は一連のヘルパー関数を持っていて、これらはゲーム状態のある部分を変更するのに使うことができるものだ。

例えばプレイヤー１のオーバーロードのスタート守備値を20から30に変更したいなら、以下のようになる:
...
Solidityをよく知らない者のために説明すると、`enum`とは`uint`と簡単に変換で可能なユーザー定義のデータタイプを作成するやり方だ。
...
これを行うのは主に、コードの読みやすさと何が実際に起こっているのかをよりはっきりさせるためなのだ。
...
１番目の例では、プレイヤー１のスタッツを変更しているのが一目で簡単にわかる。一方２番目の例では`0`が何に帰するのか、コードを読んだ人は関数定義を参照しなくてはならなくなるだろう。

## Zombie Battlegroundプレイヤースタッツの変更

上の`changes.changePlayerDefense()`に加えて、`changes`オブジェクト上で呼び出してゲームのあらゆるスタッツに手を加えることのできるたくさんのメソッドが他にもある:

* `changePlayerCurrentGooVials` — 現在プレイヤーが持っている、このラウンドで有効なGooボトル数。デフォルトでは、対戦スタート時に１より始まり、毎ターン１ずつ増えていく。
* `changePlayerCurrentGoo` — 現在のラウンドにおける、Gooで満たされたボトル数。デフォルトでは、各ターンスタート時のGooボトル数と同じ。
* `changePlayerMaxGooVials` — ゲーム中でプレイヤーが持つことのできるGooボトルの最大数。デフォルトでは10。
* `changePlayerInitialCardsInHandCount` — 対戦スタート時にプレイヤーに配られるカード数。デフォルトでは３。
* `changePlayerMaxCardsInPlay` — プレイヤーがフィールドに置くことのできるゾンビの最大数。デフォルトでは６。
* `changePlayerMaxCardsInHand` — プレイヤーが一度に手札に持って置くことのできるカードの最大数。デフォルトでは10。

これらの関数は全て`changePlayerDefense`と同じフォーマットに従っていて、２つの引数を受け取る: enumの`Player` と`uint8`だ。

## さあテストだ

これらの数値いくつかに手を加えて、我らがExampleGameモードのためのカスタムゲームルールを作ってみよう。

1. プレイヤー１とプレイヤー２のスタート守備値を15に設定せよ。
2. 両プレイヤーの`CurrentGooVials`を３に設定し、また両プレイヤーの`CurrentGoo`も３に設定せよ。
3. 両プレイヤーの`MaxGooVials`を８に設定せよ。

上のコードは合計８行になるはずだ。上でメソッド名を探して、全メソッドが`changes.changePlayerDefense(Player.Player1, 30);`と同じフォーマットに従っていることを思い出すのだ。
