...
このチャプターでは、関数`isLegalCard()`を定義していこう。

この関数はコード１行で定義することができるが、ではなぜ`if`ステートメントにインラインで定義せず、別途で関数を作成するのか？

その理由は、今後新たなゲームモードを作成する時このコントラクトをフォークして(このコピーを作って)、そこをスタート地点とすることができるからだ。そうするとどのカードがルールに則っているかを再定義する際に、この１つの関数の中身を変えるだけで良いので非常にわかりやすくなるのだ。

## さあテストだ

1. `isLegalCard`という関数を定義せよ。この関数は`card`という名の`CardInstance`という１つの引数を受け取って`bool`を返却し、`internal`及び`view`というプロパティを持つようにすること。

2. 関数本体では、`card.mouldName`が`bannedCards`マッピングにない場合に真を返却しなくてはならない。これは`return (!bannedCards[card.mouldName]);`で行うことができる。
