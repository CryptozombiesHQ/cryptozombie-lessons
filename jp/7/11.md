...
ネスト化された`for`ループの実行が終わったら、両プレイヤーのカードを禁止されたカードを含まない新たなセットで置き換えるんだったな(ただしプレイヤーのデッキに最初からカードがなかった場合は、何も変更されない)。

あとは各プレイヤーのスタート守備値やGooボトルの変更も行った。

しかし`changes`は単にメモリで保存されるローカルデータタイプであったことを思い出そう。我々のゲームモードを完成させるには、このデータをコアのゲームロジックへと返さなくてはならないのだ。

これは`changes.emit()`で簡単に行うことができるぞ:

```
changes.emit();
```

その名の通りこれは`event`を送信し、ゲームのコアロジックはそれをリッスンしている。

このイベントは、我々が行いたい全変更のデータを`serializedGameState`と同様の形式で含んでいる。

## さあテストだ

1. 関数を`changes.emit()`で完成させよ。
