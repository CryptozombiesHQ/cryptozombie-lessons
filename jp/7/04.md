...
チャプター３で初期化した`GameState`を使って、現在のゲーム状態からデータを読み取れるようになったな。

`beforeMatchStart()`の最後では、そのデータを`SerializedGameState`としてゲームサーバーに返す必要があるのだ。

このためには、我々が行った変更全てをトラックしてくれる`ZBSerializer.SerializedGameStateChanges`という名のカスタムデータタイプを宣言しなくてはならない。

```
ZBSerializer.SerializedGameStateChanges memory changes;
```

`changes`ではその名のとおり、この関数の最後で返却されるゲーム状態に全ての変更を行う。

これを宣言したら、初期化も行う必要がある:

```
changes.init();
```

これでデータ保存を始める準備が整うのだ。`gameState`とは違って、`changes.init()`は引数を必要としない。

## さあテストだ

1. `changes`という`memory`で、`ZBSerializer.SerializedGameStateChanges`を宣言せよ。

2. `changes.init()`を呼び出せ。
