...
もしわしがノードを立てているとしたら、 **自分のノードだけに向けて** トランザクションを発行できる。そうするとコイン・トスの関数を動かして勝ち負けを見て、わしの解いている次のブロックにそのトランザクションが含まれないよう選択することができてしまう。わしはこれをコイン・トスに勝つまで無制限に続けることができ、勝ったらこのトランザクションを含めた次のブロックを解き、利益を得ることが可能だ。
