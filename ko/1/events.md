...
우리의 컨트랙트가 거의 완성되어 가는군! 이제 ***이벤트***를 추가해 보세.

***이벤트***는 자네의 컨트랙트가 블록체인 상에서 자네 앱의 프론트 단에서 무언가 액션이 발생했을 때 의사소통하는 방법이지. 컨트랙트는 특정 이벤트가 일어나는지 "귀를 기울이고" 그 이벤트가 발생하면 행동을 취하지.
