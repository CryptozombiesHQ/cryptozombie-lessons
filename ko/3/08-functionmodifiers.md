...
훌륭해! 우리 좀비가 이제 재사용 대기 시간 타이머를 가지게 되었군.

다음으로, 우리는 헬퍼 메소드를 좀 더 추가할 것이네. 자네를 위해 `zombiehelper.sol`이라는, `zombiefeeding.sol`을 import하는 새로운 파일을 추가해뒀네. 이렇게 하면 우리의 코드가 잘 정리된 상태를 유지할 수 있을 것이네.
