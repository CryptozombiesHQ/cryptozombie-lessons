        ...
        <body>
          ...
          <script>
            ...
            function feedOnKitty(zombieId, kittyId) {
              // 시간이 꽤 걸릴 수 있으니, 트랜잭션이 보내졌다는 것을
              // 유저가 알 수 있도록 UI를 업데이트해야 함
              $("#txStatus").text("크립토 키티 먹는 중.. 시간이 조금 걸릴지도 몰라요..");
              // 우리 컨트랙트에 전송하기:
              return CryptoZombies.methods.feedOnKitty(zombieId, kittyId)
              .send({ from: userAccount })
              .on("receipt", function(receipt) {
                $("#txStatus").text("크립토 키티를 먹고 새로운 좀비가 탄생했습니다!");
                // 블록체인에 트랜잭션이 반영되었으며, UI를 다시 그려야 함
                getZombiesByOwner(userAccount).then(displayZombies);
              })
              .on("error", function(error) {
...
2. `feedOnKitty` 라는 또 다른 함수를 구현해보겠네.
  ...
  b) `#txStatus` 텍스트를 다음과 같이 업데이트하게: `"크립토 키티 먹는 중.. 시간이 조금 걸릴지도 몰라요.."`

  c) 우리 컨트랙트의 `feedOnKitty`를 호출하게 하고, 같은 2개의 인자를 전달하게.

  d) `#txStatus`에 성공 메세지로 `"크립토 키티를 먹고 새로운 좀비가 탄생했습니다!"`를 쓰게.
