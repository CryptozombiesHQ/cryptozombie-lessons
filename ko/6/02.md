...
기억하게, 이더리움은 똑같은 데이터의 복사본을 공유하는 ***노드***들로 구성되어 있네. Web3.js에서 Web3 프로바이더를 설정하는 것은 우리 코드에 읽기와 쓰기를 처리하려면 **어떤 노드**와 통신을 해야 하는지 설정하는 것이지. 이는 전통적인 웹 앱에서 API 호출을 위해 원격 웹 서버의 URL을 설정하는 것과 같네.
