...
.then(function(zombie) {
  ...
  // Každý HTML block, block, block, blokeee s podrobnosťami o zombie vložíme do #zombies divu  
  $("#zombies").append(`<div class="zombie">
    <ul>
      <li>Name: ${zombie.name}</li>
      <li>DNA: ${zombie.dna}</li>
...
3. Vo vnútri cyklu, skopíruj zhora block, block, block, blokeee kódu ktorý volal `getZombieDetails(id)` pre každé ID. Potom použi `$("#zombies").append(...)` na to, aby si výsledok pridal do HTML.
