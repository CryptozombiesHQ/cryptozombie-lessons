...
Tento tutoriál by nebol kompletný, keby sme ti neukázali ako vlastne môžme zobraziť dáta ktoré dostaneš naspäť z kontraktu.

Každopádne, v reále by si mal najsôr použit moderný front-endový framework ako napríklad React alebo Vue.JS. Robia život front-end programátora oveľa ľahší. Diskusia Reactu alebo Vue.js je však nad rámec tohoto tutoriálu. Tomu by sa dal venovať jeden celý osobitný tutoriál, prípadne aspoň niekoľko lekcií. 
...
Spomeň si, že v predchádzajúcej kapitole sme z funkcie `startApp()` volali funkciu `displayZombies`. Tá parametrom príjma výsledok volania `getZombiesByOwner`, čo bude pole identifikačných čisiel zombie. Niečo ako:

```
[0, 13, 47]
```

Takže potrebujeme aby `displayZombies` spravila nasledovné:

1. Najprv zmazať obsah `#zombies` divu, v prípade že tam už niečo je. (Takže ak si užívateľ prepne aktívny účet v Metamasku, pred tým že na stránke zobrazia nový zombie, zombie patriaci predošlého účtu zmiznú).

2. Prejdeme cez všetky `id`, a pre každé z nich zavoláme `getZombieDetails(id)`. Tak z našeho smart kontraktu obdržíme všetky detailné informácie o danom zombie.

3. Detailné informácie o zombie zobrazíme do HTML šablóny ktorá ich naformátuje, a výsledok prilepíme do `#zombies` divu.

Budeme používať len knižnicu JQuery, ktorá nemá špeciálnu podporu na šablónovanie, preto výsledok vzhľadovo nebude vyzerať moc pekne. Dáta o každom zombie môžme teda zobrazovať napríklad takto:
...
V príklade vyššie zobrazujeme DNA jednoducho ako reťazec znakov. Ale v našej DAppke chceme ho skonvertovať na jednotlivé obrázky, aby sme zombieho zobrazili vizuálne.
...
Každá komponenta je potom umiestnená pomocou CSS absolute positioningu, tak aby sa jednotlivé obrázky pekne prekrývali.
...
1. Prvá vec ktorú vnej musíme spraviť je vyprázdniť `#zombies` div. Pomocou JQuery to môžme spraviť takto: `$("#zombies").empty();`.
