...
Pecka! Podarilo sa ti napísať prvý front-endový kód, ktorý bude komunikovať s tvojím smart kontraktom.
Awesome! You've successfully written front-end code to interact with your first smart contract.

Teraz poďme poskladať dokopy ďalšie kúsky skladačky - dajme tomu že chceme na hlavnej stránke zobrazovať celú armádu daného užívateľa.
...
Ale náš Solidity kontrakt očakáva že `owner` Solidity adresa (Solidity typ `address`). Ako môžme zistiť adresu používateľa ktorý práve používa našu appku?
...
My môžme zistiť, aký účet má užívateľ momentálne aktívny cez  `web3` premennú takto:
...
Pretože užívateľ môže svoj účet v MetaMasku kedykoľvek prepnúť, náš kód musí monitorovať túto premennú. Keď užívateľ zmení účet, mali by sme adekvátne aktualizovať obsah stránky. Ak je napríklad užívateľ práve na stránke zobrazujúcej jeho zombie armádu, ak si zmení účet v Metamasku, mali by sme zobraziť armádu patriacu účtu ktorý si v MetaMasku zvolil. 
...
Toto možme zabezpečiť pomocou cyklu `setInterval` nasledovným spôsobom:
...
Tento kód robí to, že každých 100 milisekúnd skontroluje či sa `userAccount` stále rovná hodnote `web3.eth.accounts[0]` (ak áno, užívateľ ma zvolený stále rovnaký účet.) Ak nie, aktualizuje hodnotu `userAccount` na aktuálne aktívny účet v MetaMasku a aktualizuje stránku.
...
Poďme aplikáciu upraviť tak, aby najprv zobrazila zombie armádu keď sa stránka po prvý krát načíta, a potom bude monitorovať MetaMask. V prípade že zachytí zmenu aktívneho MetaMask účtu, stránka sa opäť aktualizuje.
...
1. Deklaruj premennú `var userAccount`, no nenastavuj je zatiaľ žiadnu hodnotu.
1. Declare a `var` named `userAccount`, but don't assign it to anything.

2. Na konci deklaruj `startApp()`, skopíruj šablónu pre `accountInterval` z kódu vyššie
...
4. Po volaní `getZombiesByOwner` použi `then` aby si preposlal výsledok z Ethereum uzlu do funkcie `displayZombies`. (Syntax je takáto: `.then(displayZombies);`).
