...
Ako vidíš, interakcia s našim kontraktom cez Web3.js je celkom priamočiara. Web3 si na počiatku raz nakonfiguruješ, a potom už len ťaháš dáta z kontraktu pomocou `call`, a odosielaš kontraktu transakcie pomocou `send`. Nie je to zas tak obrovský rozdiel oproti klasickému webovému API.
...
Zostáva ešte jeden aspekt kontraktov, ktorý nám zostáva pokryť - ako sledovať udalosti vyvolané na kontrakte.
...
Ber na vedomie, že tento kód bude spustený zakaždým, keď bude v našom kontrakte vytvorený ľubovlný zombie - nie len aktuálnym užívateľom. Ale čo ak chceme udalosti týkajúceho sa aktuálneho užívateľa?
...
V tomto prípade, pretože `_from` a `_to` sú `indexed`, znamená to že môžeme podľa týchto parametrov udalosti filtrovať:
...
Ako vidíš, používanie udalostí s `indexed` je šíkovný spôsob ako sledovať zmeny ktoré nastanú v tvojom kontrakte a reflektovať ich naspať do front endu aplikácie.
...
Je možné taktiež pristupovať ku starým udalostiam, prostredníctvom funkcie `getPastEvents`. Zároveň môžme použiť filtre `fromBlock` and `toBlock`, aby sme určili rozsah časový logov udalostí. ("block" referuje na číslo Ethereum bloku):
...
.then(function(events) {
  ...
  // Tento kód by nám vrátil list všetkých zombie ktorí kedy boli vyvorený
  // `events` is an array of `event` objects that we can iterate, like we did above
  // This code will get us a list of every zombie that was ever created
});
```

Túto metódu teda možno použiť na to, aby si získal zoznam určitého typu udalostí od počiatku času. To prezentuje zaujímavý spôsob možnosti použitia udalostí: **Používanie udalostí ako lacnej formy dátového úložiska**
Because you can use this method to query the event logs since the beginning of time, this presents an interesting use case: **Using events as a cheaper form of storage**.

Ak si spomínaš, ukladanie dát na blockchain je jedna z najdrahších operácií v Solidity. No používanie udalostí je z hľadiska spotreby gasu omnoho lacnejšie.
...
Nevýhoda použitia udalostí, ako formy úložiska dát je tá, že k dátam starých udalostí nemôžeme pristupovať zo smart kontraktu samotného. Je doležité si tento spôsob využitia udalostí pamätať, v prípade že chceš aby tvoj kontrakt zaznamenával nejaké historické dáta, ktoré by si mohol využiť na strane front endu aplikácie.
...
Napríklad by sme mohli prostredníctvo udalostí zaznamenávať na blockchain výsledky všetky zombie zápasov. Proste by sme pri každom zápase zaznamenali udalosť o tom, kto koho napadol, a ktorý zombie vyhral. Smart kontrakt samotný takéto dáta vôbec nepotrebuje, no sú to užitočné dáta, ktoré by užívatelia mohli sledovať na front ende našej webovej aplikácie.
...
Tento kúod budeme musieť pridať na koniec funkcie `startApp`, aby sme zaručili, že kontrakt `cryptoZombies` bol nainicializovaný, pred tým než sa na odber udalostí prihlásime.
...
1. Skopíruj kód bloku vyššie, v ktorom načúvame na udalosti `cryptoZombies.events.Transfer`, a vlož tento kód na koniec funkcie `startApp()` 
1. At the end of `startApp()`, copy/paste the code block above listening for `cryptoZombies.events.Transfer`

2. Na riadok na ktorom by sme mali aktualizovať UI, použijeme kód `getZombiesByOwner(userAccount).then(displayZombies);`
