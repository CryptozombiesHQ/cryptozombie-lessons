...
5. Na záver vytvoríme inštanciu našeho kontraktu. Hodnotu `cryptoZombies` priraď referenciu na novú inštanciu `web3js.eth.Contract`. Presne tak, ako sme to urobili v ukážkovom kóde vyššie (ako parameter použi `cryptoZombiesABI`, ku ktorému si dostal prístup importovanim prostredníctvo script tagu. Ao druhý použi parameter premennú `cryptoZombiesAddress`).
