...
1. วิธีแรกจะให้ owner ของ token นั้น ๆ เรียกใช้ `transfer` พร้อมกับ `address` ที่ต้องการจะส่งไปให้ และ `_tokenId` ของ token ที่ต้องการส่ง
...
ดังนั้นหากสังเกตดี ๆ ทั้ง `transfer` และ `takeOwnership` จะใช้ transfer logic ที่เหมือนกันเพียงแค่เป็นไปในทางตรงกันข้ามเท่านั้นเอง (กรณีแรกผู้ส่ง token จะเป็นคนเรียกฟังก์ชั่น; อีกกรณีคือให้ผู้รับ token เป็นคนเรียกแทน)

จึงค่อนข้างที่จะเข้าใจได้ง่าย ๆ สำหรับการใช้แนวคิดนี้ใน private function นั่นก็คือ `_transfer` ซึ่งจะถูกเรียกโดยทั้งสองฟังก์ชั่น ทำให้เราไม่จำเป็นต้องเขียนโค้ดเดิมถึงสองรอบ
