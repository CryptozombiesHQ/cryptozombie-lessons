...
2. Inside `check_above_level` read `self.zombies(&zombie_id)` into a variable `my_zombie`

3. Next check to make sure `my_zombie.level` is greater than or equal to `level` or return an error message "Zombie is too low level" otherwise.
