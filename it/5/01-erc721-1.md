...
Se sei stato nello spazio di Ethereum per un certo periodo di tempo, probabilmente hai sentito persone parlare di token, in particolare ***token ERC20***.
...
Un ***token*** su Ethereum è fondamentalmente solo un contratto intelligente che segue alcune regole comuni — ovvero implementa un insieme standard di funzioni condivise da tutti gli altri contratti di token, come `transferFrom(address _from, address _to, uint256 _tokenId)` e `balanceOf(address _owner)`.
...
C'è un altro token standard che si adatta molto meglio ai cripto-collezionabili come CryptoZombies — e si chiama ***token ERC721.***

***I token ERC721*** sono **non** intercambiabili poiché ognuno è considerato unico e non divisibile. Puoi scambiarle solo in unità intere e ognuna ha un ID univoco. Quindi sono perfetti per rendere negoziabili i nostri zombi.
