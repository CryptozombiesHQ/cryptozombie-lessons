    ...
    startingCode:
      "zombieownership.sol": |
        ...
        contract ZombieOwnership is ZombieAttack, ERC721 {
          ...
          // 1. Aggiungere qui il modificatore di funzione
          function approve(address _approved, uint256 _tokenId) external payable {
            // 2. Definire la funzione qui
          }
...
Ricorda, con `approve` il trasferimento avviene in 2 passaggi:

1. Tu, il proprietario, chiami `approve` e gli fornisci l'indirizzo `_approved` del nuovo proprietario ed il `_tokenId` che vuoi trasferirgli

2. Il nuovo proprietario chiama `transferFrom` con `_tokenId`. Successivamente, il contratto verifica che sia già stato approvato, quindi trasferisce il token.

Poiché ciò accade in 2 chiamate di funzioni, dobbiamo usare la struttura dati di `zombieApprovals` per memorizzare chi è stato approvato per le chiamate tra le funzioni.

## Facciamo una prova

1. Per la funzione `approve`, vogliamo assicurarci che solo il proprietario del token possa dare a qualcuno l'approvazione per accettarlo. Quindi dobbiamo aggiungere il modificatore `onlyOwnerOf` per `approve`

2. Per il body della funzione, impostare `zombieApprovals` per `_tokenId` uguale all'indirizzo `_approved`.
