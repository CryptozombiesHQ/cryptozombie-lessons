...
Bene, ora possiamo implementare SafeMath su tutti i tipi di `uint` che abbiamo usato nella nostra DApp!

Risolviamo tutti quei potenziali problemi in `ZombieAttack`. (C'era anche uno `zombies[_zombieId].level++;` che doveva essere risolto in `ZombieHelper`, ma ci siamo occupati di quello per te, quindi non prendiamo un capitolo in più per farlo 😉).

## Facciamo una prova

Vai avanti ed implementa i metodi SafeMath su tutti gli incrementi `++` in `ZombieAttack`. Abbiamo lasciato dei commenti nel codice per renderli facili da trovare.
