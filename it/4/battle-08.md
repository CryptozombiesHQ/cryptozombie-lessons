...
Ora che abbiamo `winCount` e `lossCount`, possiamo aggiornarli a seconda dello zombi che vince il combattimento.

Nel capitolo 6 abbiamo calcolato un numero casuale da 0 a 100. Ora usiamo quel numero per determinare chi vince il combattimento e aggiorna le nostre statistiche di conseguenza.
...
2. Se questa condizione è vera, il nostro zombi vince! Così:
  ...
  b. Incrementa `myZombie`'s `level`. (Sale di Livello!!!!!!!)

  c. Incrementa `enemyZombie`'s `lossCount`. (Perdente!!!!!! 😫 😫 😫)

  d. Esegui la funzione `feedAndMultiply`. Controlla `zombiefeeding.sol` per vedere la sintassi per chiamarla. Per il terzo argomento (`_species`), passa la stringa `"zombie"`. (In realtà non fa nulla al momento, ma in seguito potremmo aggiungere funzionalità extra se volessimo generare zombi basati su zombi).
