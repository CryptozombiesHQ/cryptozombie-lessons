    ...
    startingCode:
      "zombiehelper.sol": |
        ...
        contract ZombieHelper is ZombieFeeding {
          ...
          // 1. Modifica questa funzione per usare `ownerOf`:
          function changeName(uint _zombieId, string _newName) external aboveLevel(2, _zombieId) {
            require(msg.sender == zombieToOwner[_zombieId]);
            zombies[_zombieId].name = _newName;
          }

          // 2. Fai lo stesso con questa funzione:
...
Abbiamo un altro paio di posti in `zombiehelper.sol` dove dobbiamo implementare il nostro nuovo `modifier` `ownerOf`.

## Facciamo una prova

1. Aggiorna `changeName()` per usare `ownerOf`

2. Aggiorna `changeDna()` per usare `ownerOf`
