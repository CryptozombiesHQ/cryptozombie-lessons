...
```
if zombie_coins[&caller] > 100000000 {
  // You rich!!!
} else {
...
1. Add an `else` statement. If our zombie loses:
  ...
  a. Update `self.zombies(&zombie_id)` incrementing `my_zombie.loss_count`.

  b. Update `self.zombies(&target_id)` incrementing `enemy_zombie.win_count`.
