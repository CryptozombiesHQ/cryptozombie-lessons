...
2. If this condition is true, our zombie wins! So:
  ...
  d. Run the `feed_and_multiply` function. Check `zombie_feeding.rs` to see the syntax for calling it. For the 3rd argument (`species`), pass the string `"zombie"`. (It doesn't actually do anything at the moment, but later we could add extra functionality for spawning zombie-based zombies if we wanted to).
