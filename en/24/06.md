...
## The answer
We'll go back to `zombie_helper.rs`.
...
1. Create a function called `check_zombie_belongs_to_caller`. It will take 2 arguments, `zombie_id` (a `usize`) and `caller` (a `&ManagedAddress`).

  The body should contain the require from the code above with caller being received as parameter.

> Note that, in order to avoid cloning the address of the caller, we will pass it and compare it as a reference.

2. Change the function definition of `change_dna` and `change_name` to use this function instead of the straight up call.
