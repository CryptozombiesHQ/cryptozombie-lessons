...
We will continue working on `ZombieHelper` providing some a way to change some zombie features.

## Put it to the test

1. Create an endpoint called `change_name`. It will take 2 arguments: `zombie_id` (a `usize`), and `new_name` (a `ManagedBuffer` ). It should call the `above_level` function with `2` for the `level` parameter and the `zombie_id`.

2. In this function, first we need to save `self.blockchain().get_caller()` inside a variable named `caller` and verify that the caller is equal to `zelf.zombie_owner(&zombie_id).get()` or return an error `"Only the owner of the zombie can perform this operation"`. Use a `require!` statement.

3. After the `require` assuming that everything is good, the function should set update the name of the zombie to `new_name`.
