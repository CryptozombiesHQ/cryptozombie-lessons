...
2. Add a `require` statement to check that the owner of the token matches the `address _from`.

3. We have 2 mappings that will change when ownership changes: `ownerZombieCount` (which keeps track of how many zombies an owner has) and `zombieToOwner` (which keeps track of who owns what).
...
4. Next, we'll need to **decrease** the `ownerZombieCount` for the person **sending** the zombie (`address _from`). Use `--` to decrement.

5. Lastly, we'll want to change `zombieToOwner` mapping for this `_tokenId` so it now points to `_to`.

6. I lied, that wasn't the last step. There's one more thing we should do.
