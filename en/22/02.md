...
Addresses in MultiversX Rust framework are used with the `ManagedAddress` data type as seen in the example above. Note that we are not accessing an account by being straight up a `ManagedAddress`, but rather a reference to it. In Rust, ownership is important as we discussed in lesson 1 so we will try as much as possible not to clone unnecessary data when it can be reused.
