...
1. Define a module proxy called `crypto_kitties_proxy` and put the `get_kitty` endpoint inside it. Don't forget to add also `multiversx_sc::imports!();` and `multiversx_sc::derive_imports!();` to the proxy module together with the `#[type_abi]` annotation and the `#[derive(NestedEncode, NestedDecode, TopEncode, TopDecode)]` macro to the `Kitty` struct.
