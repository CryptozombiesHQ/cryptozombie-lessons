...
2. Inside the body the first line should update `zombies_count`. We will access it's content under the name `id`. Here `id` doesn't need to be dereferenced, since `zombies` requires a `&usize` in order to access a zombie.
  
3. Insize the update closure we create new zombie and put it inside `self.zombies(id)`.
