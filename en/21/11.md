...
```
In the MultiversX Rust framework the parameters of an event need to be preceded by the `#[indexed]` annotation. Maximum 1 parameter is allowed to be without the `#[indexed]` annotation which will be considered as data.
...
2. Modify the `create_zombie` function:
   ...
   * be aware that `id` needs to be dereferenced here since we have it inside update as a reference
