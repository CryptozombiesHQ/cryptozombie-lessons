...
When we were ready to deploy this contract to Ethereum, we would just compile and deploy `ZombieFeeding` — since this contract is our final contract that inherits from `ZombieFactory`, and has access to all the public methods in both contracts.

Let's look at an example javascript implementation of how we would interact with our contract using web3.js:
...
// E.g. When the user clicks on a kitty:
...
ZombieFactory.NewZombie(function(error, result) {
  if (error) return
  // Assuming we have a function like lesson 1 that will display the zombie:
