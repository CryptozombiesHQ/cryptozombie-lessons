...
我们在第3章初始化的`GameState`现在可用于从当前游戏状态读取数据了。

在`beforeMatchStart（）`结束时，我们需要将数据作为`SerializedGameState`传递回游戏服务器。

为此，我们需要声明一个名为`ZBSerializer.SerializedGameStateChanges`的自定义数据类型，它将跟踪我们所做的所有更改。
...
这就准备好开始存储数据了。 与`gameState`不同，`changes.init（）`不需要任何参数。

## 实战演习

1. 在`内存`中声明一个名为`changes`的`ZBSerializer.SerializedGameStateChanges`。
