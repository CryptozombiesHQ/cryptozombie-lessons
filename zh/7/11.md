...
等我们的嵌套`for`循环结束运行后，我们会已经将双方玩家的卡牌换成新的一套不含任何禁牌的牌组。（如果玩家一开始牌组中就没有工具牌，就不会产生任何变化）。

我们还对每个玩家的初始防御值和goo瓶数进行了一些更改。

但请记得，`changes`只是存储在内存中的本地数据类型。要完成我们的游戏模式，我们需要将这些数据传递回核心游戏逻辑。

这可以简单地用`changes.emit()`完成：

```
changes.emit();
```

正如该名称所描述的那样，它将发出一个游戏核心逻辑正在监听的`event`。

这个事件包含我们要进行的所有更改的数据，格式类似于`serializedGameState`。

## 实战演习

1. 完成我们的`changes.emit()`函数。
