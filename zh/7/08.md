...
接着上一章的内容，现在我们要遍历访问玩家牌组中的每一张卡牌来检查它们是否被禁用了。

## 循环访问玩家牌组中的卡牌

以下是一个例子说明如何在两个玩家的牌组中遍历每张卡牌，并将它们全部更改为“Slab” - 一张粉丝们最爱的卡牌 😉
...
正如你所见，我们使用嵌套`for`循环首先遍历列表中的每个玩家，然后再遍历每个玩家的每张卡牌。

> 注意：更改玩家牌组中卡牌的`mouldName`实际上会将该卡牌完全换成名为Slab的牌。 它不仅仅是一个显示名称。

## 实战演习

1. 创建第二个`for`循环，遍历每个玩家的所有`cardsInDeck`，如上所示。

2. 创建一个调用`isLegalCard`函数的`if`语句，并给它传递`gameState.playerStates[i].cardsInDeck[j]`。我们将在下一章定义这个函数。

3. 在这个`if`语句中，通过设置`newCards [cardCount]`等于`gameState.playerStates [i] .cardsInDeck [j]`将卡牌复制到我们的新数组。

4. 使用`cardCount ++`来增加`cardCount`。
