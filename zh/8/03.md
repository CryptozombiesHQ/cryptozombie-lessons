...
*Munchkin*的最后一步是填充`isLegalCard()`的函数定义 —— 然后这个游戏模式就完全实现了！

超级简单，对吧？

回想一下*Munchkin*的规则，我们要**禁止任何goo值超过2的卡牌**。

要查看过滤这些卡牌的最佳方法，让我们再看一下`CardInstance`：
...
如你所见，我们有一个`gooCost`属性。 因此，要测试一张卡牌在*Munchkin*中是否合法，我们可以直接检查`card.gooCost`是否小于或等于2。

## 实战演习

由于`isLegalCard()`返回`bool`，因此可以使用一行代码填充此函数定义 —— `return`语句。

如果`card.gooCost`小于或等于2，该函数应返回`true`，反之则返回`false`。

1. 使用`return`语句填充函数定义。
