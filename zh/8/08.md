...
现在我们已经为每张卡牌实现了随机抛一次硬币，代码将根据抛掷结果将该卡牌添加到其中一个玩家的牌组中。

但是有个问题 —— 如果抛硬币的结果让1号玩家获得的超过一半的卡牌，怎么办？
 
这不仅对游戏不公平 —— 还会使得我们之前创建用来容纳卡牌的数组溢出 🙃

因此，我们需要在代码中添加一项检查，以确保不会发生这种情况。

## 实战演习

在我们的`if`语句之前，添加**另一个**`if`语句，来检查我们是否将溢出数组。

1. 添加一个`if`语句，检查`player1CardCount + 1`是否大于`gameState.playerStates[0].cardsInDeck.length`。

2. 如果是，则设置`rand = 1`（这样我们后面的if / else语句将把卡牌给2号玩家而不是1号玩家）。

3. 添加`else if`子句，检查`player2CardCount + 1`是否大于`gameState.playerStates[1].cardsInDeck.length`。

4. 如果是，则设置`rand = 0`（将卡牌给1号玩家）。
