...
1. First, we want to make sure only the owner or the approved address of a token/zombie can transfer it. Let's define a mapping `zombieApprovals`. It should map a `uint` to an `address`.
