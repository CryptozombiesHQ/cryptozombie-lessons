...
En Solidity, il existe des variables globales accessibles à toutes les fonctions. L'une d'elles est `msg.sender`, qui fait référence à l'`address` de la personne (ou du smart contract) qui a appelé la fonction actuelle.

> Remarque : En Solidity, l'exécution d'une fonction nécessite obligatoirement un appel extérieur. Un contrat va juste rester dans la blockchain à ne rien faire jusqu'à ce que quelqu'un appelle une de ses fonctions. Il y aura toujours un `msg.sender`.
...
Utiliser `msg.sender` apporte de la sécurité à la blockchain Ethereum - la seule manière pour quelqu'un de modifier les données d'un autre serait de lui voler la clé privée associée à son adresse Ethereum.
