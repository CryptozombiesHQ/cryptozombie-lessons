...
Vous remarquerez que cela va déclencher une alerte pour N'IMPORTE quel zombie créé dans notre DApp - et pas seulement pour l'utilisateur courant. Et si nous voulions seulement des alertes pour l'utilisateur courant ?

## Utiliser `indexed` (indexé)

Afin de filtrer les évènements et écouter seulement les changements liés à l'utilisateur courant, notre contrat Solidity devra utiliser le mot clé `indexed`, c'est ce que nous avons fait avec l'évènement `Transfer` de notre implémentation ERC721 :

```
event Transfer(address indexed _from, address indexed _to, uint256 _tokenId);
```

Dans ce cas, puisque `_from` et `_to` sont `indexed`, cela veut dire que nous pouvons les utiliser comme filtre dans notre écouteur d'évènement de notre front-end :
...
Nous allons ajouter du code pour écouter l'évènement `Transfer`, et mettre à jour notre interface si l'utiisateur actuel reçoie un nouveau zombie.

Nous allons avoir besoin d'ajouter ce code à la fin de la fonction `startApp`, pour être sûr que le contrat `cryptoZombies` soit bien initialisé avant d'ajouter l'écouteur d'évènement.

1. À la fin de `startApp()`, copier/coller le bloc de code ci-dessous qui écoute pour un `cryptoZombies.events.Transfer`
