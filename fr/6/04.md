...
Maintenant nous allons voir un vrai exemple de la fonction `call` pour accéder aux donnés de notre contrat.
...
En Solidity, quand vous déclarer une variable `public`, cela crée automatiquement une fonction "getter" (une fonction de récupération) public avec le même nom. Si vous voulez récupérer le zombie avec l'id `15`, vous l’appellerez comme si c'était une fonction : `zombies(15)`.
...
2. Après cela, créez une troisième fonction pour `getZombiesByOwner`. Si vous vous rappelez de `ZombieHelper.sol`, la définission de la fonction était :
