...
`call` est utilisée pour les fonctions `view` et `pure`. Elle est exécutée seulement sur le nœud local, et cela ne va pas créer de transaction sur la blockchain.

> **Rappel :** les fonctions `view` et `pure` sont des fonctions en lecture seule et ne changent pas l'état de la blockchain. Elles n'ont aucun coût en gas et l'utilisateur n'aura pas besoin de signer de transaction avec MetaMask.
