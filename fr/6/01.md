...
En terminant la Leçon 5, notre DApp zombie est maintenant complète. Maintenant, nous allons créer une page web basique où vos utilisateurs pourrons interagir avec.

Pour cela, nous allons utiliser une bibliothèque JavaScript de la fondation Ethereum appelée **_Web3.js_**.

## Qu'est-ce Web3.js ?

Rappelez-vous, le réseau Ethereum est fait de nœuds, dont chacun contient une copie de la blockchain. Quand vous voulez appeler une fonction d'un smart contract, vous avez besoin de demander à l'un de ces nœuds en lui indiquant :

 1. L'adresse du smart contract
 2. La fonction que vous voulez appeler, et
 3. Les variables que vous voulez donner à la fonction.

Les nœuds Ethereum parlent seulement un langage appelé **_JSON-RPC_**, qui n'est pas vraiment lisible par l'homme. Une requête pour demander à un nœud que vous voulez appeler une fonction d'un contrat ressemble à ça :
```
...
Nous avons créer un fichier HTML pour vous, `index.html`. Supposons que nous avons une copie de `web3.min.js` dans le même dossier qu'`index.html`.
