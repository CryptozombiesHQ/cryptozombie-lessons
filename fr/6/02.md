...
Rappelez-vous, Ethereum est fait de **_nœuds_** que partagent une copie des mêmes données. Configurer un fournisseur Web3 indique à notre code avec **quel nœud** nous devrions communiquer pour traiter nos lectures et écritures. C'est un peu comme configurer l'URL d'un serveur web distant pour des appels API d'une application web classique.

Vous pourriez héberger votre propre noeud Ethereum comme fournisseur. Mais il existe un service tiers qui vous facilitera la vie pour que vous n'ayez pas besoin de vous occuper de votre propre nœud Ethereum pour fournir une DApp à vos utilisateurs - **_Infura_**.

## Infura

<a href="https://infura.io/" target=_blank>Infura</a> est un service qui a plusieurs noeuds Ethereum avec une couche de cache pour des lectures plus rapides, que vous pouvez accéder gratuitement depuis leur API. En utilisant Infura comme fournisseur, vous pouvez envoyer et recevoir des messages de la blockchain Ethereum de manière fiable, sans avoir besoin de vous occuper de votre propre nœud.
...
Voici un modèle de code fourni par Metamask pour détecter si l'utilisateur a Metamask installé ou non, et sinon leur dire qu'il doit l'installer pour utiliser notre application :
...
1. Faites un copier/coller du code modèle ci-dessus pour détecter Metamask. C'est le block qui commence par `window.addEventListener`.
