...
Vous allez avoir besoin de copier cette adresse après avoir le déploiement afin de pouvoir communiquer avec le smart contract.
...
Si nous incluons `cryptozombies_abi.js` dans notre projet, nous pourrons accéder l'ABI de CryptoZombies en utilisant cette variable.
...
2. Au début de notre balise `<script>` dans le `<body>`, déclarez une `var` appelée `cryptoZombies`, mais ne lui attribué pas de valeur. Nous allons nous servir de cet variable plus tard pour stocker notre contrat instancié.

3. Ensuite, créez une `function` appelée `startApp()`. Nous la compléterons dans les 2 prochaines étapes.

4. La première chose que `startApp()` doit faire et de déclarer une `var` appelée `cryptoZombiesAddress` égale à `"YOUR_CONTRACT_ADDRESS"` (c'est l'adresse du contrat sur le réseau principal).
