...
Oups ! Nous venons juste d'ajouter une erreur dans notre code qui l’empêche de compiler. Est-ce que vous l'avez vue ?

Dans le chapitre précédent, nous avons défini une fonction `ownerOf`. Mais si vous vous rappelez de la leçon 4, nous avons aussi créé un `modifier` avec le même nom, `ownerOf`, dans `zombiefeeding.sol`.
