...
De esa forma no repetiremos el mismo código dos veces.
...
Si te das cuenta, tanto `transfer` como `takeOwnership` contienen la misma lógica para la transferencia, sólo que en orden inverso.
...
Notice that both methods contain the same transfer logic. (En un caso, el emisor del token llama a la función, en el otro el receptor del token lo llama).

Entonces, tiene sentido que abstraigamos esta lógica en su propia función privada, `_transfer`, que luego es llamada por ambas funciones.
