...
> Nota: Recuerda, `uint256` es equivalente a `uint`. We've been using `uint` in our code up until now, but we're using `uint256` here because we copy/pasted from the spec.
