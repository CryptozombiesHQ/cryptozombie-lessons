      ...
      zombieownership.sol: |
        ...
        contract ZombieOwnership is ZombieAttack, ERC721 {
          ...
          function balanceOf(address _owner) external view returns (uint256 _balance) {
            // 1. Devuelve el numero de zoombies `_owner` que tienes aqui
          }

          function ownerOf(uint256 _tokenId) external view returns (address _owner) {
            // 2. Devuelve a el dueño de `_tokenId` aqui
          }
...
¡Profundicemos en la implementación de ERC721!
...
1. Implemente `balanceOf` para que devuelva el número de zombies que `_owner` tiene.

2. Implemente `ownerOf` para que devuelva la dirección del dueño del zombi con ID  `_tokenId`.
