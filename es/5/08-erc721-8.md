...
2. Para llamar a `_transfer`, necesitamos saber la dirección de la persona que posee el token (requiere `_from` como un argumento). Afortunadamente, podemos buscar esto con nuestra función `ownerOf`.
    
    Así que, declara una variable `address` llamada `owner`, y colócala igual a `ownerOf(_tokenId)`.

3. Finalmente, llama a `_transfer`, y pase toda la información necesaria. (Aquí puede usar `msg.sender` para el valor `_to`, ya que la persona que llama a esta función es a la que debe enviarse el token).

> Nota: Podríamos haber hecho los pasos 2 y 3 en una línea de código, pero dividirla hace que las cosas sean un poco más legibles. Preferencia personal.