      ...
      zombieownership.sol: |
        ...
        contract ZombieOwnership is ZombieAttack, ERC721 {
          ...
          function transfer(address _to, uint256 _tokenId) external payable {
            // 2. Así que añade el `modifier` llamado `onlyOwnerOf` a esta función. Agrega la declaracion adecuada aqui
            // 3. Llama a _transfer
          }
...
1. Queremos asegurarnos de que solo el dueño de un token / zombie pueda transferirlo. Let's define a mapping called `zombieApprovals`. It should map a `uint` to an `address`.
...
2. Next, let's add a `require` statement to `transferFrom`. Sólo necesita llamar a `_transfer`.
