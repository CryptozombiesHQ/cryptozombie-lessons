...
1. Primero, definamos el mapping `zombieApprovals`. Deberá ser un map de tipo `uint`  con una dirección `address`.
...
3. En el cuerpo de la función, establezca `zombieApprovals` para `_tokenId` con la dirección de `_to`.

4. Finalmente, hay un evento `Approval` en la especificación ERC721. Entonces, debemos activar este evento al final de esta función. Revise los parámetros en `erc721.sol` , y asegúrese de utilizar `msg.sender` para el campo `_owner`.
