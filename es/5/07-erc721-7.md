  ...
  editor:
    ...
    startingCode:
      "zombieownership.sol": |
        ...
        // 2. Añade el modificador de función aquí
        function approve(address _to, uint256 _tokenId) public {
        // 3. Define la función aquí
        }
...
Como esto ocurre durante 2 llamadas distintas, necesitamos una estructura de datos para almacenar quién ha sido aprobado para cada llamada de una función.

## Vamos a probarlo

1. Primero, vamos a definir un mapping `zombieApprovals`. Deberá mapear un `uint` con una dirección `address`.
...
2. En la función `approve`, queremos asegurarnos de que solo el propietario del token pueda dar la aprobación para que otra persona pueda tomar el token. Así que necesitaremos el modifier de `onlyOwnerOf` para usar `approve`

3. En el cuerpo de la función, establece `zombieApprovals` para `_tokenId` con la dirección de `_to`.

4. Finalmente, hay un evento `Approval` en la especificación ERC721. Entonces, debemos lanzar este evento al final de esta función. Revisa los argumentos `erc721.sol`, y asegúrate de utilizar `msg.sender` como `_owner`.