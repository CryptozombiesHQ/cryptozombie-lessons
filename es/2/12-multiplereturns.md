...
3. La función tendrá entonces que llamar a la función `kittyContract.getKitty` con `_kittyId` y guardar `genes` en `kittyDna`. Recuerda — `getKitty` devuelve un montón de variables. (10 para ser exactos - Soy bueno, ¡los he contado por ti!). Pero el único que nos interesa es el último de ellos, `genes`. Count your commas carefully!
