...
1. Dar a nuestro contrato un `uint` llamado `randNonce`, y configurarlo como igual a `0`.

2. Crear una función llamada `randMod` (random-modulus). Será una función `internal` que tome un `uint` llamado `_modulus`, y `returns` un `uint`.
