...
Usando Web3.js, deberías `call` una función llamada `myMethod` con el parámetro `123` de la siguiente manera:
...
> **Nota:** `send`ing una transacción va a requerir al usuario pagar gas, y aparecerá su Metamask para solicitarle que firme su transacción. Cuando usamos Metamask como nuestro proveedor web3, todo esto sucede automáticamente cuando llamamos a `send()`, Así que no necesitaremos hacer nada especial en nuestro código. ¡Genial!
